...
const FilterCondition = class {
  ...
  renderContent(condition) {
    ...
    switch (condition.type) {
      ...
      case 'NUMBER':
        ...
        return (index.h("fw-input-condition", { key: `${this.filterOn}-${this.identifier}`, value: this.value, controlProps: Object.assign(Object.assign({}, this.controlProps), props), ref: (conditionElement) => (this.conditionElement = conditionElement) }));
      case 'NUMBER_RANGE':
        return (index.h("fw-input-range-condition", { key: `${this.filterOn}-${this.identifier}`, value: this.value, controlProps: this.controlProps, ref: (conditionElement) => (this.conditionElement = conditionElement) }));
      case 'DROPDOWN':
        return (index.h("fw-select-condition", { key: `${this.filterOn}-${this.identifier}`, value: this.value, controlProps: this.controlProps, ref: (conditionElement) => (this.conditionElement = conditionElement) }));
      case 'MULTI_SELECT':
        props['multiple'] = true;
        return (index.h("fw-select-condition", { key: `${this.filterOn}-${this.identifier}`, value: this.value, controlProps: Object.assign(Object.assign({}, this.controlProps), props), ref: (conditionElement) => (this.conditionElement = conditionElement) }));
      case 'DATE':
        props = {
        ...
        return (index.h("fw-date-condition", { key: `${this.filterOn}-${this.selectedCondition}`, controlProps: Object.assign(Object.assign({}, this.controlProps), props), ref: (conditionElement) => (this.conditionElement = conditionElement) }));
      case 'DATE_RANGE':
  ...
  render() {
    return (index.h("div", { class: 'container' }, index.h("div", { class: 'header' }, index.h("div", { class: 'header-text' }, index.h("span", { class: 'displayText' }, this.filterText), index.h("fw-filter-dropdown", { id: 'condition-select', value: this.condition, options: this.conditionOptions })), index.h("span", { onClick: () => {
        this.fwDelete.emit({ value: this.filterOn });
      }, class: 'closeBtn' }, index.h("fw-icon", { name: 'cross', color: '#264966', size: 8 }))), index.h("div", { class: 'content' }, this.renderContent(this.conditionSchema[this.selectedCondition]))));
  }
...
const SearchDropdown = class {
  ...
  componentWillLoad() { }
  render() {
    return (index.h("fw-popover", { ref: (dropdown) => (this.dropdown = dropdown), distance: '4', "same-width": 'true', placement: 'bottom' }, index.h("fw-input", { slot: 'popover-trigger', placeholder: Translation.TranslationController.t('searchDropdown.placeholder.search'), iconRight: 'search', tabIndex: 0, role: 'button' }), index.h("fw-list-options", { slot: 'popover-content', filterText: this.value, options: this.options })));
  }
  get host() { return index.getElement(this); }
};
SearchDropdown.style = searchDropdownCss;

exports.fw_filter_condition = FilterCondition;
exports.fw_search_dropdown = SearchDropdown;

...
const index$1 = require('./index-62345b50.js');
const index = require('./index-147690f5.js');
...
class Draggable {
  constructor(container, options) {
    ...
    this.acceptFrom = [];
    this.dropped = false;
    this.debouncedSetElement = index.debounce((childElements, draggingElement, y) => {
      ...
      if (this.placeholder) {
        if (this.options.addOnDrop) {
          const clone = this.options.copy
            ? index.cloneNodeWithEvents(dragElement, true, true)
            : dragElement;
...
const DragContainer = class {
  constructor(hostRef) {
    ...
    this.fwDrop = index$1.createEvent(this, "fwDrop", 7);
    /**
  ...
  render() {
    return index$1.h(index$1.Host, { class: 'drag-container' });
  }

...
const v4 = require('./v4-cf5aaa67.js');
...
const Filter = class {
  constructor(hostRef) {
    ...
    this.fwFilterChange = index.createEvent(this, "fwFilterChange", 7);
  ...
  renderContent(filterConditions) {
    return Object.keys(filterConditions).map((filterOn) => {
      ...
      return (index.h("fw-filter-condition", Object.assign({ key: filterOn }, defaultProps)));
    });
  }
  componentWillLoad() {
  ...
  render() {
    return (index.h("div", { class: 'filter' }, this.isFiltersAvailable() && (index.h("div", { class: 'filter-content' }, this.renderContent(this.filterConditions))), index.h("div", { class: 'filter-action' }, this.showSearch ? (index.h("fw-search-dropdown", { id: 'searchDropdown', options: this.filterOptions })) : (this.filterOptions.length !== 0 && (index.h("span", { class: 'add-filter', role: 'button', tabIndex: 0, onClick: () => {
        this.showSearch = true;
      }, onKeyDown: utils.handleKeyDown(() => {
        this.showSearch = true;
      }) }, Translation.TranslationController.t('filters.addFilter'))))), !this.isFiltersAvailable() && (index.h("slot", { name: 'filter-empty-state' }, index.h("div", { style: {
        display: 'flex',
        height: '100%',
        alignItems: 'center',
        justifyContent: 'center',
      } }, index.h("span", null, Translation.TranslationController.t('filters.noFilter')))))));
  }

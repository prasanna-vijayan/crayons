...
const FbFieldDropdown$1 = class {
  constructor(hostRef) {
    ...
    this.validate = () => {
      if (this.dataProvider && this.dataProvider.length > 0) {
        this.errorType = '';
        const strDuplicateErrorKey = formBuilderUtils.i18nText('errors.duplicate');
        const arrChoices = formBuilderUtils.deepCloneObject(this.dataProvider);
        let boolElementUpdated = false;
        const arrLookup = [];
        arrChoices.map((el) => {
          const strValue = el.value;
          if (!strValue || strValue === '') {
            this.errorType = formBuilderUtils.i18nText('errors.formErrors');
            el.error = formBuilderUtils.i18nText('errors.emptyChoice');
            ...
            if (formBuilderUtils.hasCustomProperty(el, 'error')) {
              delete el.error;
              boolElementUpdated = true;
            }
        ...
        this.errorType = formBuilderUtils.i18nText('errors.minimum');
      }
    };
    this.validateMaximumChoiceLimits = () => {
      const objMaxLimitChoices = formBuilderUtils.getMaxLimitProperty(this.productName, 'maxDropdownChoices');
    ...
    this.addNewChoiceHandler = (event) => {
      ...
      if (this.level) {
        objNewChoice['id'] = formBuilderUtils.createUUID();
        objNewChoice['dependent_ids'] = { choice: [], field: [] };
      }
      this.dataProvider = [...this.dataProvider, objNewChoice];
      this.errorType = formBuilderUtils.i18nText('errors.emptyChoice');
      this.validateMaximumChoiceLimits();
      this.fwChange.emit({
        type: 'ADD',
    ...
    this.deleteItemHandler = (event) => {
      ...
      if (intDeleteIndex > -1 &&
        ...
        if (!isNewChoice && (!this.errorType || this.errorType === '')) {
          this.errorType = formBuilderUtils.i18nText('errors.deleteDropDownChoice');
        }
        this.fwChange.emit({
          type: 'DELETE',
          errorType: this.errorType,
    ...
    this.choiceValueChangeHandler = (event) => {
      ...
      if (strValue === '') {
        this.dataProvider[intIndex].error = formBuilderUtils.i18nText('errors.emptyChoice');
      }
    ...
    this.elementDropHandler = (event) => {
      ...
      if (objDetail.dragFromId === objDetail.dropToId &&
        ...
        for (let i1 = intDroppedIndex; i1 < intLength; i1++) {
          ...
          arrFields[i1].repositionKey = formBuilderUtils.createUUID();
        }
  ...
  renderNameEditorElement(dataItem, intIndex) {
    const hasRepositionIndex = formBuilderUtils.hasCustomProperty(dataItem, 'repositionKey');
    const boolNewChoice = !formBuilderUtils.hasCustomProperty(dataItem, 'id');
  ...
  render() {
    ...
    const strExceedLimitChoicesWarning = this.boolExceededChoiceLimit
      ? formBuilderUtils.i18nText(objMaxLimitChoices.message, {
        count: objMaxLimitChoices.count,
      })
      : '';
    const labelText = this.isDependentField
      ? `${formBuilderUtils.i18nText('level')} ${this.level}`
      : formBuilderUtils.i18nText('addChoices');
    ...
    const dropdownClass = this.isDependentField
      ...
      : strBaseClassName;
    return (index.h(index.Host, { tabIndex: '-1' }, index.h("div", { class: dropdownClass }, index.h("label", { class: labelClass }, labelText), index.h("fw-drag-container", { id: 'dropdownElementsList', class: `${strBaseClassName}-list-container`, sortable: true, onFwDrop: this.elementDropHandler }, dropdownElements), index.h("div", { class: `${strBaseClassName}-footer` }, !this.disabled && !this.enableKeyPress && (index.h("fw-button", { id: 'addNewChoiceBtn', color: 'link', disabled: this.boolExceededChoiceLimit, onFwClick: this.addNewChoiceHandler }, formBuilderUtils.i18nText('addChoice'))), this.boolExceededChoiceLimit && (index.h("label", { class: `${strBaseClassName}-warning-text` }, strExceedLimitChoicesWarning))))));
  }
...
const FbFieldDropdownItem = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.fwAdd = index.createEvent(this, "fwAdd", 7);
  ...
  render() {
    ...
    const strErrorMsg = formBuilderUtils.hasCustomProperty(dpSource, 'error')
      ? dpSource.error
      : '';
    ...
    if (strErrorMsg === formBuilderUtils.i18nText('errors.duplicate')) {
      showFieldNameError = true;
    }
    const strBaseClassName = 'fb-field-dropdown-item';
    const formattedInputPrompt = `${this.toOrdinalSuffix(this.index + 1).toString()} ${formBuilderUtils.i18nText('choicePlaceholderSuffix')}`;
    const strInputPrompt = this.isDependentField
      ? formBuilderUtils.i18nText('addChoice')
    ...
    const dropdownItemClass = this.isDependentField
      ...
      : strBaseClassName;
    return (index.h(index.Host, { tabIndex: '-1' }, index.h("div", { class: dropdownItemClass, ref: (el) => (this.divBaseElement = el), onDragEnd: this.stopParentDragging, onDrop: this.stopParentDragging }, index.h("div", { role: 'none', class: strDragClassName, onMouseDown: this.startParentDragging }, index.h("fw-icon", { size: 14, name: 'drag', color: '#475867' })), index.h("div", { class: `${strBaseClassName}-input-container` }, index.h("fw-input", { class: `${strBaseClassName}-content-required-input`, state: showFieldNameError ? 'error' : 'normal', errorText: strErrorMsg, placeholder: strInputPrompt, value: dpSource.value, disabled: this.disabled, onFwBlur: this.nameBlurHandler, onFwInput: this.nameChangeHandler, onFwFocus: this.nameFocusHandler, onFwInputKeyDown: this.nameKeydownHandler })), index.h("span", { class: strDeleteClassName, onClick: this.deleteButtonClickHandler }, index.h("fw-icon", { name: 'delete', size: 14 })))));
  }
  get host() { return index.getElement(this); }
};
...
const FbFieldDropdown = class {
  constructor(hostRef) {
    ...
    this.isNewField = () => {
      if (formBuilderUtils.hasCustomProperty(this.formValues, 'isNew') &&
    ...
    this.relationshipChangeHandler = (event) => {
      ...
      this.fwChange.emit({ value: formBuilderUtils.deepCloneObject(this.dataResponse) });
    };
  ...
  watchValuesChangeHandler() {
    var _a;
    const arrRelationshipTypes = formBuilderPreset.presetSchema.relationshipTypes;
    let relationshipValue = '';
    if (this.formValues && !this.isNewField()) {
      const relTargetId = formBuilderUtils.hasCustomProperty(this.formValues, 'related_entity_id')
      ...
      const boolOneToOne = formBuilderUtils.isUniqueField(this.formValues);
  ...
  componentWillLoad() {
    ...
    if (!this.i18RelationshipType) {
      ...
      for (let i1 = 0; i1 < intLength; i1++) {
        this.i18RelationshipType = [
          ...this.i18RelationshipType,
          Object.assign(Object.assign({}, arrRelationshipTypes[i1]), { text: formBuilderUtils.i18nText(arrRelationshipTypes[i1].text), subText: formBuilderUtils.i18nText(arrRelationshipTypes[i1].subText) }),
        ];
      }
    }
  ...
  render() {
    ...
    if (this.selectedRelationshipValue &&
      ...
      this.targetObjectLabel &&
      this.targetObjectLabel !== '') {
      const objRelationshipDesc = formBuilderPreset.presetSchema.relationshipDescriptionKeys[this.selectedRelationshipValue];
      if (objRelationshipDesc) {
        const strDescKey = this.isNativeTargetObject
          ? objRelationshipDesc.native
          : objRelationshipDesc.co;
        strDescription = formBuilderUtils.i18nText(strDescKey, {
    ...
    return (index.h(index.Host, { tabIndex: '-1' }, index.h("div", { class: `${strBaseClassName}-root` }, index.h("label", { class: `${strBaseClassName}-header-label` }, formBuilderUtils.i18nText('lookupAssociationHeader')), index.h("div", { class: strBaseClassName }, index.h("fw-input", { class: `${strBaseClassName}-input`, label: formBuilderUtils.i18nText('lookupSourceObject'), value: this.sourceObjectName, disabled: true }), boolShowRelationshipTypeSelect ? (index.h("div", { class: `${strBaseClassName}-relationship-select-container` }, index.h("fw-select", { readonly: true, required: true, state: strRelationshipState, class: `${strBaseClassName}-relationship-select`, placeholder: formBuilderUtils.i18nText('lookupRelationshipPlaceholder'), label: formBuilderUtils.i18nText('lookupRelationshipLabel'), errorText: formBuilderUtils.i18nText('errors.emptyRelationshipType'), disabled: !boolNewField || this.disabled, options: this.i18RelationshipType, value: this.selectedRelationshipValue, onFwChange: this.relationshipChangeHandler }))) : (index.h("div", null)), index.h("div", { class: `${strBaseClassName}-target-select-container` }, index.h("fw-select", { required: true, state: strTargetState, class: `${strBaseClassName}-target-select`, placeholder: formBuilderUtils.i18nText('lookupTargetPlaceholder'), label: formBuilderUtils.i18nText('lookupTargetLabel'), errorText: formBuilderUtils.i18nText('errors.emptyTargetObject'), disabled: !boolNewField || this.disabled, value: this.selectedTargetValue, options: this.targetObjects, onFwChange: this.targetObjectChangeHandler }))), boolShowDescription && (index.h("span", { class: `${strBaseClassName}-relationship-description`, innerHTML: strDescription })))));
  }
  get host() { return index.getElement(this); }
  static get watchers() { return {
...
exports.fw_fb_field_dropdown = FbFieldDropdown$1;
exports.fw_fb_field_dropdown_item = FbFieldDropdownItem;
exports.fw_fb_field_lookup = FbFieldDropdown;
